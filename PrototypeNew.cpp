...
void dominance()
{
	string emotions[11] = { "Joy(Face)","Fear","Disgust","Sadness","Anger(Face)","Surprise","Contempt","Calm","Anger","Joy","Sorrow" };
	cout << "Doubling Empath Values" << endl;
	for (int i = 0;i < 34;i++)
	{
		for (int k = 0;k < 4;k++)
		{
			int j = 7 + k;
			combinedvalues[i][j] = combinedvalues[i][j] * 2;
		}
	}
	string dominant[34];
	int max = 0;
	int index = 0;
	cout << "Compute dominant emotion" << endl;
	for (int k = 0;k<34;k++)
	{
		cout << "Computing Dominant emotion for Sample:" << k << endl;
		for (int i = 0;i<11;i++)
		{
			if (combinedvalues[k][i]>max)
			{
				max = combinedvalues[k][i];
				index = i;
			}

		}
		dominant[k] = emotions[index];
		max = 0;
		cout << "Dominant emotion for sample " << k << " is " << dominant[k] << endl;
	}
}
...
case 59:
	...
	else if (deccomlog == true)
	{
		...
		newcsv = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\NewCSV.csv";
		inputarff = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\NewCSV.arff";
		newCSV(newcsv);
		j48Model = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\J48Model.model";
		nnModel = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\NNModel.model";
		svmModel = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\SVMModel.model";
		nbModel = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\NBModel.model";
		output = "C:\\Users\\LENOVO\\Desktop\\Thesis\\CombinedDatasets\\Output.csv";
		WekaModelApplication(newcsv, inputarff, j48Model, nnModel, svmModel, nbModel, output);
